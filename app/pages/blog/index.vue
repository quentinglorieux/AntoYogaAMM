    ...
    <div class="py-12 bg-gray-50 min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-12">
                <h1 class="text-3xl font-light text-gray-900 sm:text-4xl">Le Blog</h1>
                <p class="mt-4 text-lg text-gray-500">Articles, conseils et actualités autour du Yoga et de la Montagne
                </p>
            </div>

            <!-- Loading State -->
            <div v-if="status === 'pending'" class="text-center py-12">
                <UIcon name="i-heroicons-arrow-path" class="w-8 h-8 animate-spin mx-auto text-teal-600" />
                <p class="mt-4 text-gray-500">Chargement des articles...</p>
            </div>
            ...
            <div v-else-if="articles && articles.length > 0" class="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                <NuxtLink v-for="article in articles" :key="article.path" :to="article.path"
                    ...
                    <div class="flex-1 flex flex-col p-6">
                        <div class="flex-1">
                            <p class="text-sm font-medium text-teal-600 mb-2">
                                <span v-if="article.date">{{ new Date(article.date).toLocaleDateString('fr-FR', {
                                    year:
                                        'numeric', month: 'long', day: 'numeric'
                                }) }}</span>
                            </p>
                            <h3 class="text-xl font-semibold text-gray-900 group-hover:text-teal-600 transition-colors">
                                {{ article.title }}
                            </h3>
                        ...
                        <div class="mt-6 flex items-center text-teal-600 font-medium group-hover:text-teal-700">
                            Lire la suite <span
